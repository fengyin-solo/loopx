...
| --- | --- | --- |
| 管家 M1 — 可用的宿主 agent | 通道解析并回报其生效执行器、模型与来源；没有 chat 传输的宿主以 typed reason 失败，而不是静默回落到个人登录 | 仅存在于托管栈（PR #4409、#4416、#4417）：凭据解析的端点与模型、`dsh_chat_transport_unsupported`、managed executor 读取，以及被 gate 的 `managed_host_chat_transport_unsupported` 请求。上游**会话身份**尚未投影到通道，因此通道回答还无法证明是哪一次会话给出的 |
...
| 宿主模式 M0-M1 | 通道的执行器选型与其有界单段执行 | 由本文覆盖：凭据解析的绑定与上面的 Mode B 验收 |
